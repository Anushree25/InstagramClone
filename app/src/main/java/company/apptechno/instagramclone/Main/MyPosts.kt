...
data class PostRow(
    var post1: PostDetails? = null,
    var post2: PostDetails? = null,
    var post3: PostDetails? = null
) {
    fun isFull() = post1 != null && post2 != null && post3 != null
    fun add(post: PostDetails) {
        if (post1 == null) {
            post1 = post
        } else if (post2 == null) {
            post2 = post
...
@Composable
fun MyPostsScreen(navController: NavController, vm: MainViewModel) {
    ...
    val posts = vm.postsUploaded.value
...
fun ProfileImage(imageUrl: String?, onClick: () -> Unit) {
    Box(modifier = Modifier
        ...
        Card(
            ...
            Image(
                painter = painterResource(id = company.apptechno.instagramclone.R.drawable.ic_add_user),
...
@Composable
fun PostsRow(item: PostRow, onPostClick: (PostDetails) -> Unit) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .height(120.dp)
